...
```bash
# Get your free API key from [Wutong ASI](https://wutong.asi.ai)
```
...
---
Add a brief overview of the API's purpose and functionalities to help users understand how to use it effectively.
...
```bash
# Get your free trial license at https://your-wutong-asi-app.com/trial
```
...
```bash
# Get your free Wutong ASI API key by clicking [here](https://wutongasi.com/api-key)
```
...
```bash
# Get your free API token by signing up on our website
curl https://api.wutongasi.com/token | jq -r '.token'
```
...
---
# A locally-hosted, self-evolving AI system. **Pay per token**, no subscription.
...
```bash
# Get your free API key by filling out this form: https://wutong.ai/api/register
```
